...
namespace FuzzUtils
{
  ...
  // ============================================================================
    ...
    private:
        bool CanExtract(size_t size) const { return offset_ + size <= size_; }
...
const APIDescriptor g_apiTable[] = {
...
constexpr int32_t g_apiCount = sizeof(g_apiTable) / sizeof(g_apiTable[0]);
...
void ExecuteRandomMode(DataExtractor &extractor)
{
    ...
    {
        int32_t apiIndex = extractor.ExtractInt32() % g_apiCount;
        if (apiIndex >= 0 && apiIndex < g_api_count)
        {
            g_apiTable[apiIndex].func(extractor);
        }
    }
}
...
void ExecuteSequentialMode(DataExtractor &extractor)
{
    ...
    {
        for (int32_t i = 0; i < g_api_count; ++i)
        {
            if (g_apiTable[i].groupId == group && extractor.HasMore())
            {
                g_apiTable[i].func(extractor);
            }
        }
    }
}
...
void ExecuteStressMode(DataExtractor &extractor)
{
    ...
    {
        int32_t apiIndex = (extractor.ExtractInt32() + callCount) % g_apiCount;
        if (apiIndex >= 0 && apiIndex < g_api_count)
        {
            g_apiTable[apiIndex].func(extractor);
        }
